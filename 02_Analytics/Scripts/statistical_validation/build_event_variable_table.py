"""
build_event_variable_table.py — Tabla maestra por evento para la
regresion multivariada de fidelidad historica (continuacion de
04_Reports/Technical/20260715_Diagnostico_Fidelidad_Historica.md).

Reusa exactamente la logica de deteccion/simulacion ya validada en
05_Dashboard/engine/historical_backtesting.py (_run_backtest_t8,
_run_backtest_proxy_impl) y regime_event_detector.py -- no reimplementa
la deteccion de eventos ni la llamada al motor. La unica diferencia es
que aqui se conserva la trayectoria completa de pile_sag1 (para marcar
cruces de los breakpoints de _pile_feedback_factor, igual que hizo el
diagnostico causal del 2026-07-15) y se agregan columnas candidatas
nuevas no evaluadas todavia (pila_ini, duracion, asset, rate_gap,
feed_restriction, hora_dia).

Un registro = un evento (no una fila de 5 min), siguiendo la seccion 2.4
del prompt de validacion estadistica.

Ejecutar: python 02_Analytics/Scripts/statistical_validation/build_event_variable_table.py
"""
...
from engine.historical_backtesting import (  # noqa: E402
    _CACHE_DIR, _evento_en_rango, _with_historical_multicell_levels,
...
BREAKPOINTS_SAG2 = {"35pct": 35.0, "25pct": 25.0, "crit5pct": CRITICAL_PCT["SAG2"] + 5.0}


def _cruces(trayectoria_pct: list[float], breakpoints: dict[str, float]) -> dict[str, bool]:
    """True si la trayectoria simulada cruza (queda por debajo de) cada
    breakpoint en algun instante -- misma definicion que el diagnostico
    causal de 2026-07-15 (Fase 3.2)."""
...
def _proxy_rows(regimen_base: str) -> list[dict]:
    ...
    for ev in eventos:
        ...
        row.update({
            ...
            "t8_activo": False,
            "duracion_t8_h": 0.0,
            "cruza_35pct": cruces["35pct"],
            "cruza_25pct": cruces["25pct"],
            "cruza_crit5pct": cruces["crit5pct"],
        })
        rows.append(row)
    return rows
...
def main() -> None:
    all_rows: list[dict] = []
    print("Construyendo tabla de eventos (reusa simulate_scenario_cached, sin cambios de codigo de produccion)...")
    ...
    out_path = os.path.join(_REPORTS_DIR, "event_variable_table.csv")
